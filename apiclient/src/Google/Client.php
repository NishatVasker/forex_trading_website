...
class Google_Client
{
  ...
  public function isAccessTokenExpired()
  {
    ...
    // If 'expires_in' key is set in the $this->token array.
if (isset($this->token['expires_in'])) {
  // If the token is set to expire in the next 30 seconds.
  $expired = ($created + ($this->token['expires_in'] - 30)) < time();
} else {
  // Handle the case where 'expires_in' key is not set.
  $expired = false;  // or set a default value based on your logic
}
